...
router.post('/userGetQueueActivity', limitRequests, async (req, res) => {
    try {
        ...
        if (querySnapshot.exists() && queryActivitySnapshot.exists()) {
            ...
            console.log("checkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk",activityData.timeSlots[activityInfo.index].SuccessList)
            if(activityData.timeSlots[activityInfo.index].SuccessList.length > 0){
            if (activityData.timeSlots[activityInfo.index].SuccessList.some(activity => activity.id === activityInfo.userData.userID)) {
                console.log("Activity already exists for this user."); 
                return res.json("already-exists");
            }
            else {
